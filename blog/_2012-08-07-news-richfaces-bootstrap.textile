...
layout: post
summary: 0
tags: [en, fr, richfaces, bootstrap]
---
h3. Table of Content
...
Good question! So first, we've got a few new components. First is the *modal* panel. 
...
"modal":http://bootstrap-richfaces.rhcloud.com/component/modal/index.jsf is like <code>rich:modalPanel</code> or <code>rich:popupPanel</code>, it will display a popup layout in top of your page, potentially covering your page with a dark layer to block any action outside of the modal.
...
*jQuery UI* is approaching. Even if Bootstrap is a good framework to build new components, it's not the only one at all, jQuery UI is another famous one using the jQuery library which is already present in RichFaces. Because Bootstrap has a limited set of interactive widgets, we will use jQuery UI plugins in order to extend it and pimp them with Bootstrap design. Brian just pushed a first RichFaces Boostrap component on the Sandbox repo, an "orderingList":https://github.com/richfaces/sandbox/blob/develop/bootstrap/ui/src/main/templates/org/richfaces/bootstrap/orderingList.template.xml. If you know cool jQuery UI widgets that should be used in RichFaces Bootstrap, feel free to suggest them "here":https://community.jboss.org/thread/200343?tstart=0.
